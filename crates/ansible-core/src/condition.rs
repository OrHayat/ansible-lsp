...
///
/// The root only: `daos_mount_check.stat.exists` yields `daos_mount_check`, because
...
mod tests {
    ...
    fn skip_flag_runs_unless_set() {
        let v = classify("not (skip_samba | default(false) | bool)");
        assert_eq!(v, Verdict::UnlessSet { var: "skip_samba".into() });
        assert_eq!(v.label().unwrap(), "runs unless skip_samba is set");
    }
    ...
    /// The mutually-exclusive pair from playbooks/daos-deploy-full.yml.
    #[test]
    fn equality_against_the_default_value() {
        let native = classify("daos_deployment_mode | default('native') == 'native'");
        let docker = classify("daos_deployment_mode | default('native') == 'docker'");
        assert_eq!(native.var(), Some("daos_deployment_mode"));
        assert_eq!(native.label().unwrap(), "runs unless daos_deployment_mode changes from native");
        assert_eq!(docker.label().unwrap(), "runs only if daos_deployment_mode = docker");
    ...
    fn anything_with_real_logic_is_unknown() {
        for c in [
            ...
            "'smb' in (ec_container_expose | default(['nfs', 'smb']))",
            "daos_role == \"storage\"",
        ] {
    ...
    fn every_requirement_shape_renders() {
        ...
        assert_eq!(
            v.label().unwrap(),
            "runs only if demo_mode != docker and proto in [nfs, smb] and \
    ...
    fn extracts_root_variables_only() {
        assert_eq!(variables("daos_mount_check.stat.exists"), vec!["daos_mount_check"]);
        assert_eq!(
    ...
    fn guarded_conditions_swallow_undefined_variables() {
        ...
        assert!(!is_guarded(&["daos_role == 'storage'".into()]));
        // Every clause must guard itself.
        assert!(!is_guarded(&[
            "skip_x | default(false)".into(),
            "daos_role == 'storage'".into()
        ]));
        assert!(!is_guarded(&[]));
    }
}
...
mod corpus {
    ...
    fn repo() -> PathBuf {
        PathBuf::from(std::env::var("HOME").unwrap()).join("matrix/ansible")
    }
