...
mod tests {
    ...
    fn nested_task_file_resolves_against_role_tasks_dir() {
        ...
        let f = format!("{home}/matrix/ansible/roles/daos-snapshot/tasks/query/timestamp.yml");
        if !Path::new(&f).exists() {
            return;
        }
        let c = FileContext::discover(Path::new(&f));
        assert!(c.role_dir.as_ref().unwrap().ends_with("roles/daos-snapshot"));
        assert!(
            c.role_tasks_dir
                .as_ref()
                .unwrap()
                .ends_with("roles/daos-snapshot/tasks")
        );
        assert!(c.project_root.as_ref().unwrap().ends_with("matrix/ansible"));
