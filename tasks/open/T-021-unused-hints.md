...
A grep approximation over `~/matrix/ansible` suggested 4 roles and ~74 task files are never
...
- 2 of the 3 collections have `galaxy.yml` (`volumez/daos`, `volumez/host` — `volumez/spdk`
  does not)
- of the 4 roles a first grep flagged as unreferenced, 3 have no `meta/` at all — but
  **`spdk-daemon` does, with `galaxy_info`**, so it would be exempted despite looking unused
...
- [ ] nothing with `galaxy_info` / `galaxy.yml` is ever flagged — including `spdk-daemon`,
