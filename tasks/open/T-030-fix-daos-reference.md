...
| ------ | -------- | ---- | ------------------ |
| open   | P1       | S    | `~/matrix/ansible` — **not this one** |

## Problem

```
site.yml:6  roles/daos   <- does not exist
```

`site.yml` names a role `daos` that isn't on any `roles_path` entry. It is the only missing
...
Not a code change here — a fix in `~/matrix/ansible`, and someone with the history has to
decide which:
...
`roles/daos-snapshot`, `roles/daos-nvme-binding` and a `daos-cluster` reference all exist, so
...
- [ ] `./target/release/scan ~/matrix/ansible` exits zero
