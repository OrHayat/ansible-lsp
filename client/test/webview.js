...
const render = (candidates, current, dest, auto, autoResolved) =>
...
function panel(candidates, current) {
  const p = runPanel(scriptOf(render(candidates, current)));
  p.save = () => {
...
check("the empty state says whether the automatic default finds anything", () => {
  ...
  assert(
    !none.els.sel.children.some((c) => (c.dataset || {}).tip),
    "the empty box repeats the note in a tooltip"
  );
});
